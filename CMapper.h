...
struct SCell
{
  ...
  SCell(int xmin, int xmax, int ymin, int ymax):iTicksSpentHere(0), visited(0)
  {
    Cell.left   = xmin;
    Cell.right  = xmax;
    Cell.top    = ymin;
  ...
  bool operator==(const SCell& other) const {
      return iTicksSpentHere == other.iTicksSpentHere &&
          visited == other.visited &&
          Cell.left == other.Cell.left &&
          Cell.right == other.Cell.right &&
          Cell.top == other.Cell.top &&
          Cell.bottom == other.Cell.bottom;
  }
  bool operator!=(const SCell& other) const {
      return !(*this == other);
  }
