...
vector<double> Cga::GetWeights() {
    vector<double> weights;
    for (int i = 0; i < m_vecGenomes.size(); i++) {
        vector<SLinkGene> gene = m_vecGenomes[i].Genes();
        weights.push_back(gene[0].dWeight);
    }
    return weights;
}
...
vector<CNeuralNet*> Cga::Epoch(const vector<double>& FitnessScores)
{
    ...
    {
        MessageBox(NULL, ("Cga::Epoch(scores" + to_string(FitnessScores.size()) + "genomes mismatch)!").c_str(), "Error", MB_OK);
    }
    ...
    if (NumSpawnedSoFar < CParams::iNumOnScreen)
    {

        //calculate amount of additional children required
        int Rqd = CParams::iNumOnScreen - NumSpawnedSoFar;
...
void Cga::SafeOutputDebugString(const std::string& msg)
{
    try
    {
    ...
    {
        std::cerr << "OutputDebugString failed: " << e.what() << std::endl;
    }
}
...
//--------------------------- GetNodeWeights ----------------------------
//
//  Returns a vector of node-specific weights (if applicable). For example,
//  you might return biases or activation responses for each neuron in the genome.
//------------------------------------------------------------------------
